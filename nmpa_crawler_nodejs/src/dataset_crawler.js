...
const DETAIL_COOLDOWN_MS = Math.max(0, parseInt(process.env.NMPA_DETAIL_COOLDOWN_MS ?? `${BROWSER_COOLDOWN_MS}`, 10));
...
class BrowserController {
        ...
        while (true) {
            if (this.currentPage) {
                try {
                    ...
                    this.logger.info(`导航失败需等待 ${BROWSER_COOLDOWN_MS}ms 后重试现有浏览器`);
                    await sleep(BROWSER_COOLDOWN_MS);
                    await this.navigateWithRetries(this.currentPage);
                    continue;
                }
            }
            try {
    ...
    async navigateWithRetries(page) {
        ...
        while (true) {
            try {
                ...
                const wait = NAVIGATION_COOLDOWN_MS * Math.max(1, attempt);
                this.logger.warning(`导航失败（${message}），等待 ${wait}ms 后原页重试（第 ${attempt} 次）`);
                await sleep(wait);
                if (!page || page.isClosed?.()) {
                    if (!this.currentContext) throw error;
    ...
    async cooldownAndRestart(reason, cooldownMs) {
        if (cooldownMs > 0) {
            this.logger.info(`${reason}，等待 ${cooldownMs}ms 后重试当前浏览器`);
        ...
        const handleSegment = async (segment) => {
            ...
            while (attempt < SEGMENT_RETRY_LIMIT) {
                try {
                    ...
                    await sleep(BROWSER_COOLDOWN_MS);
                    await controller.navigateWithRetries(controller.currentPage ?? await controller.getPage());
                }
            }
        };
...
async function crawlDomesticSegment(controller, segment, { prefix, logger }) {
    ...
    try {
        while (pageNum <= segment.totalPages) {
            ...
            if (!payload) {
                ...
                try {
                    ...
                    if (error?.shouldCooldown) {
                        ...
                        logger.info(`${segment.searchValue}: 等待 ${BLOCK_COOLDOWN_MS}ms 后重试列表`);
                        await sleep(BLOCK_COOLDOWN_MS);
                        await controller.navigateWithRetries(page);
                        rotationRecoveries = 0;
                        payload = null;
                        continue;
                    }
            ...
            if (detailBlocked) {
                logger.info(`${segment.searchValue}: 详情触发冷却，等待 ${DETAIL_COOLDOWN_MS}ms 后原页重试`);
                await sleep(DETAIL_COOLDOWN_MS);
                await controller.navigateWithRetries(page);
                if (blockedId) {
                    try {
                        const recovery = await fetchDetailWithRetry(page, DOMESTIC_ITEM_ID, blockedId, DETAIL_RETRY_LIMIT + 1);
                        if (recovery && recovery.f0?.startsWith(prefix) && !seenCodes.has(recovery.f0)) {
                            const record = {
                                code: recovery.f0 || '',
                                zh: recovery.f1 || '',
                                en: recovery.f2 || '',
                        ...
                        logger.warning(`${segment.searchValue}: 冷却后再次获取记录 ${blockedId} 失败: ${cooldownError.message}`);
                    }
                }
                if (detailBlocked) {
                    payload = null;
            ...
            if (successRatio < DETAIL_SUCCESS_MIN_RATIO) {
                ...
                logger.info(`${segment.searchValue}: 等待 ${DETAIL_COOLDOWN_MS}ms 以恢复详情成功率`);
                await sleep(DETAIL_COOLDOWN_MS);
                await controller.navigateWithRetries(page);
                payload = null;
                rotationRecoveries = 0;
